...
class VulkanCommandProcessor : public CommandProcessor {
 public:
  ...
  // ── Daytona native renderer bridge ─────────────────────────────────────────

  DaytonaNativeVkObjects DaytonaGetNativeVkObjects() const override {
    DaytonaNativeVkObjects obj;
    auto* vd = GetVulkanDevice();
    if (vd) {
      obj.vk_device = reinterpret_cast<uint64_t>(static_cast<VkDevice>(vd->device()));
      obj.vk_physical_device = reinterpret_cast<uint64_t>(
          static_cast<VkPhysicalDevice>(vd->physical_device()));
    }
    return obj;
  }

  // Phase 2+: compile GLSL to SPIR-V for the daytona native pipeline.
  // Returns true on success; logs the error on failure.
  bool DaytonaNativeCompileGlsl(VkShaderStageFlagBits stage, const char* source,
                                 std::vector<uint32_t>& spirv_out) {
    std::string error;
    if (!CompileGlslToSpirv(stage, source, spirv_out, error)) {
      REXGPU_ERROR("DaytonaNative: GLSL compile failed (stage={}): {}", uint32_t(stage), error);
      return false;
    }
    return true;
  }

  // Phase 2+: attempt a fully-native draw. Lazily initializes native pipeline
  // resources, but only returns true (suppresses ReXGlue) when the matching
  // DAYTONA_NATIVE_* takeover env var, or DAYTONA_NATIVE_ALL, is enabled.
 ...
 protected:
  ...
  // ── Daytona native execution bridge ─────────────────────────────────────
  // Packages all SDK internals the project's native renderer needs so the
  // implementation can live in the project, not in the SDK. This is the
  // interface the project uses today (via ReXGlue submission) and can
  // replace entirely when the project owns its own Vulkan device.
  struct DaytonaNativeExecContext {
    ...
    // Thin wrappers that forward to VulkanCommandProcessor protected methods.
    bool BeginSubmission(bool is_guest = true) const;
    void SubmitBarriersAndEnterRenderPass(
    ...
    void BindExternalGraphicsPipeline(
        VkPipeline pipeline,
  ...
  // Project-callable entry points: thin wrappers that the project's
  // daytona_vulkan_renderer.cpp calls via the context pointer, keeping the
  // SDK implementation layer intact while the project owns the dispatch logic.
  void DaytonaNativeSetupContext();
  void DaytonaNativeShutdownContext();
  void DaytonaNativeReclaimFrame(uint64_t completed_frame);
  bool DaytonaNativeIssueDrawImpl(xenos::PrimitiveType prim_type,
                                   uint32_t index_count,
                                   const DaytonaIndexBufferInfo* ibi);
  bool DaytonaNativeIssuePointListImpl(uint32_t index_count,
                                        const DaytonaIndexBufferInfo* ibi);
  bool DaytonaNativeIssueMeshImpl(xenos::PrimitiveType prim_type,
                                   uint32_t index_count,
                                   const DaytonaIndexBufferInfo* ibi);

 private:
  // ── Daytona native pipeline (QuadList target) ────────────────────────────
  struct DaytonaNativeQuadPipeline {
    // Phase 2: textured pipeline (VS + FS with tex0/tex1)
    VkShaderModule        vert_module    = VK_NULL_HANDLE;
    VkShaderModule        frag_module    = VK_NULL_HANDLE;
    VkDescriptorSetLayout dset_layout    = VK_NULL_HANDLE;
    // Per-frame-in-flight descriptor pool ring. Texture descriptor sets are
    // allocated fresh every draw and reset when the frame slot is reused (the
    // owning frame has completed), mirroring ReXGlue's per-draw binding writes.
    // The previous cross-frame cache keyed on VkImageView handles was unsafe:
    // ReXGlue destroys/recreates (and recycles) those handles as textures churn,
    // so cached descriptors referenced freed views — corrupting/flickering the
    // car-body textures — and the 256-entry cap forced overflow draws back to
    // ReXGlue, mixing native and ReXGlue output for the same surface.
    // NOTE: literal 3 must equal kMaxFramesInFlight (declared later in this
    // class, so not yet visible as an array bound here); static_assert below.
    static constexpr uint32_t kDsetPoolRingSize = 3;
    VkDescriptorPool      dset_pool_ring[kDsetPoolRingSize] = {};
    uint64_t              dset_pool_ring_frame[kDsetPoolRingSize] = {};
    VkPipelineLayout      layout         = VK_NULL_HANDLE;
    VkPipeline            pipeline       = VK_NULL_HANDLE;
    VkPipeline            pipeline_depth = VK_NULL_HANDLE;
    // Opaque-blend variants (src=ONE dst=ZERO). Daytona's 3D track/car QuadList
    // class programs RB_BLENDCONTROL0=0x00010001 (opaque overwrite), not the
    // alpha blend used by the 2D UI class. ReXGlue builds the blend state from
    // the live register per draw; the native path must do the same or opaque
    // geometry gets alpha-blended (transparent/flickering where texel alpha<1).
    VkPipeline            pipeline_opaque       = VK_NULL_HANDLE;
    VkPipeline            pipeline_depth_opaque = VK_NULL_HANDLE;
    VkFormat              depth_vk_fmt   = VK_FORMAT_UNDEFINED;
    VkSampler             sampler        = VK_NULL_HANDLE;
    // Phase 3a: vertex-color geo-test (no descriptor sets)
    VkShaderModule        geo_frag_module = VK_NULL_HANDLE;
    VkPipelineLayout      geo_layout      = VK_NULL_HANDLE;
    VkPipeline            geo_pipeline    = VK_NULL_HANDLE;
    VkPipeline            geo_pipeline_depth = VK_NULL_HANDLE;
    // Phase 3a+: per-frame upload pool for native vertex rows. Each draw must
    // get a unique range because recorded Vulkan draws reference buffer memory
    // asynchronously.
    ...
    bool                  init_ok        = false;
  } daytona_quad_pipeline_;

  bool DaytonaNativeInitQuadPipeline();
  void DaytonaNativeDestroyQuadPipeline();

  // Native pipeline for the high-volume Daytona PointList target. This path is
  // intentionally separate from QuadList because ReXGlue expands these draws as
  // point sprites / triangle strips, not as ordinary guest vertex rows.
  struct DaytonaNativePointPipeline {
    VkShaderModule        vert_module    = VK_NULL_HANDLE;
    VkShaderModule        frag_module    = VK_NULL_HANDLE;
    VkPipelineLayout      layout         = VK_NULL_HANDLE;
    VkPipeline            pipeline       = VK_NULL_HANDLE;
    bool                  init_attempted = false;
    bool                  init_ok        = false;
  } daytona_point_pipeline_;

  bool DaytonaNativeInitPointPipeline();
  void DaytonaNativeDestroyPointPipeline();

  // Native pipeline for the Daytona TriangleStrip mesh target
  // (VS 63A523F2409137F3 / PS 7ABDFEFBCADB4BF9): 3 separate vertex streams
  // (position, color, UV; each stride 6 dwords), single texture, tex*color PS.
  struct DaytonaNativeMeshPipeline {
    VkShaderModule        vert_module    = VK_NULL_HANDLE;
    VkShaderModule        frag_module    = VK_NULL_HANDLE;
    VkDescriptorSetLayout dset_layout    = VK_NULL_HANDLE;
    VkDescriptorPool      dset_pool      = VK_NULL_HANDLE;
    VkPipelineLayout      layout         = VK_NULL_HANDLE;
    VkPipeline            pipeline       = VK_NULL_HANDLE;        // no depth, TRIANGLE_STRIP
    VkPipeline            pipeline_noclip = VK_NULL_HANDLE;       // no depth, clip disabled, TRIANGLE_STRIP
    VkPipeline            pipeline_msaa4 = VK_NULL_HANDLE;        // no depth, TRIANGLE_STRIP, 4x MSAA
    VkPipeline            pipeline_noclip_msaa4 = VK_NULL_HANDLE; // no depth, clip disabled, TRIANGLE_STRIP, 4x MSAA
    VkPipeline            pipeline_depth = VK_NULL_HANDLE;        // depth test+write (kD24S8), TRIANGLE_STRIP
    VkPipeline            pipeline_fan   = VK_NULL_HANDLE;        // no depth, QuadList as TRIANGLE_STRIP
    VkFormat              depth_vk_fmt   = VK_FORMAT_UNDEFINED;   // resolved at init time
    VkSampler             sampler        = VK_NULL_HANDLE;
    // Per-frame upload pool for decoded native mesh vertices. Recorded Vulkan
    // draws reference buffer memory asynchronously, so each draw needs its own
    // range rather than rewriting offset 0 of a persistent mapped buffer.
    std::unique_ptr<ui::vulkan::VulkanUploadBufferPool> vb_pool;
    struct TextureSet {
    ...
    bool                  init_attempted = false;
    bool                  init_ok        = false;
  } daytona_mesh_pipeline_;

  // Persistent pipeline cache shared by all three Daytona native init functions.
  // Loaded from logs/daytona_pipeline_cache.bin at startup; saved at shutdown.
